						...
						<div class="row">
							<div class="col-sm-4 col-lg-3 text-center">
								<a href="{{ route('movies.show', $result['id']) }}" 
									...
									<img class="w92" src="{!! $image->getUrl($result['poster_path'], 'w92') !!}">
								</a>
								<div class="row text-xs-center center-block">
									<a class="text-xs-center" href="{{ route('movies.show', $result['id']) }}">
						...
						@for ($i = 1; $i <= $response['total_pages']; $i++)
							@if ($i == $response['page'])
								<li class="page-item active">
									<a class="page-link" href="{{route('searchMovies', ['q' => $query, 'page' => $i])}}">
										{{ $i }}
									</a>
								</li>
							@else
